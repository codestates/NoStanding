...
import { faLocationDot, faUtensils, faMugHot } from "@fortawesome/free-solid-svg-icons";
...
const CategoryList = styled.div`
  color: ${(props) =>
    String(props.idx) === props.backgroundOn ? "rgb(101,124,241)" : "rgb(85,85,85)"};
  transition: 0.4s;
  font-size: 15px;
  font-weight: bold;
  padding-top: 1vh;
...
const CategortCityList = styled.div`
  color: ${(props) =>
    String(props.idx) === props.backgroundCity ? "rgb(251,176,64)" : "rgb(85,85,85)"};
  transition: 0.4s;
  font-size: 15px;
  font-weight: bold;
  padding-top: 1vh;
        ...
        <FlexRow>
          {category.map((category, idx) => {
            return (
              ...
              >
                {idx === 0? <FontAwesomeIcon icon={faUtensils} />:<FontAwesomeIcon icon={faMugHot} />}
                <p>{category}</p>
              </CategoryList>
            );
          })}
